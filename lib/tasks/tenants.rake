...
namespace :tenants do
  ...
  task :populate_core_data, [:path] => :environment do |_, _|
    ...
    query = "INSERT INTO #{tenant_name}.category_indicators SELECT id, name, slug, indicator_type::text::#{tenant_name}.category_indicator_type, weight, rubric_category_id, data_source, source_indicator, created_at, updated_at, script_name FROM public.category_indicators;"
    ActiveRecord::Base.connection.exec_query(query)
    
    # Create default admin user
    Apartment::Tenant.switch(tenant_name) do
      admin_user = User.new({:email => "admin@fao.org", :password => "Admin123", :password_confirmation => "Admin123" })
      admin_user.confirm
      admin_user.save
